...
#ifdef PLATFORM_MACOS9
    /*
     * Apple's Mac OS 9 OpenGL reports 1.2.x, so GLAD does not request
     * functions promoted to core in 1.3/1.4 even when the matching ARB/EXT
     * entry points are exported by OpenGLLibrary.  The legacy renderer uses
     * the core names; bridge those names explicitly after GLAD has loaded.
     */
    if (!glad_glActiveTexture)
        glad_glActiveTexture = (PFNGLACTIVETEXTUREPROC)platformGetProcAddress("glActiveTexture");
    if (!glad_glClientActiveTexture)
        glad_glClientActiveTexture = (PFNGLCLIENTACTIVETEXTUREPROC)platformGetProcAddress("glClientActiveTexture");
    if (!glad_glBlendFuncSeparate)
        glad_glBlendFuncSeparate = (PFNGLBLENDFUNCSEPARATEPROC)platformGetProcAddress("glBlendFuncSeparate");
...
#if !defined(_WIN32) && !defined(PLATFORM_MACOS9)
...
int main(int argc, char* argv[]) {
...
#endif
#ifdef PLATFORM_MACOS9
    static char* fallbackArgv[] = { "Butterscotch", nullptr };
    if (argc <= 0 || argv == nullptr || argv[0] == nullptr) { argc = 1; argv = fallbackArgv; }
#endif
#ifdef _WIN32
    timeBeginPeriod(1);
#endif
...
#ifdef PLATFORM_MACOS9
    MacTrace_stage("command line parse begin");
#endif
    parseCommandLineArgs(&args, argc, argv);
#ifdef PLATFORM_MACOS9
    MacTrace_stage("command line parse complete");
#endif
    ...
    while (true) {
...
#if defined(USE_MINIAUDIO) || defined(USE_OPENAL) || defined(USE_MACOS9_AUDIO)
        if (!args.headless)
...
#ifdef PLATFORM_MACOS9
        MacTrace_stage("data.win parse begin");
#endif
        DataWin* dataWin = DataWin_parse(currentDataWinPath, options);
#ifdef PLATFORM_MACOS9
        MacTrace_stage("data.win parse complete");
#endif
        ...
        // Initialize VM
#ifdef PLATFORM_MACOS9
        MacTrace_stage("VM create begin");
#endif
        VMContext* vm = VM_create(dataWin);
#ifdef PLATFORM_MACOS9
        MacTrace_stage("VM create complete");
#endif
...
#ifdef PLATFORM_MACOS9
            MacTrace_stage("platform and AGL init begin");
#endif
...
#ifdef PLATFORM_MACOS9
            MacTrace_stage("platform, AGL and GL symbols initialized");
#endif
        } else {
...
#endif
#ifdef PLATFORM_MACOS9
        MacTrace_stage("renderer object created");
#endif
        if (!renderer) {
...
#elif defined(USE_MACOS9_AUDIO)
            audioSystem = MacAudioSystem_create();
#else
            audioSystem = (AudioSystem*) NoopAudioSystem_create();
#endif
        }
...
#ifdef PLATFORM_MACOS9
        MacTrace_stage("Runner_create begin");
#endif
        Runner* runner = Runner_create(dataWin, vm, renderer, (FileSystem*) overlayFs, audioSystem);
#ifdef PLATFORM_MACOS9
        MacTrace_stage("Runner_create complete");
#endif
...
#ifdef PLATFORM_MACOS9
        MacTrace_stage("first room initialization begin");
#endif
        Runner_initFirstRoom(runner);
#ifdef PLATFORM_MACOS9
        MacTrace_stage("first room initialization complete; entering game loop");
#endif
