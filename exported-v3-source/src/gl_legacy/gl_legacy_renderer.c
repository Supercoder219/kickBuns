...
static bool hasFBO() {
    ...
    return glGenFramebuffers && glDeleteFramebuffers && glBindFramebuffer &&
           glFramebufferTexture2D && glCheckFramebufferStatus && glBlitFramebuffer;
#endif
}
...
static void glInit(Renderer* renderer, DataWin* dataWin) {
    ...
    gl->directFramebuffer = !hasFBO();
    if (gl->directFramebuffer) {
        fprintf(stderr, "GL: FBOs unavailable; using direct-window compatibility mode.\n");
        fprintf(stderr, "GL: User-created surfaces will be unavailable in this mode.\n");
    }
...
static void glDestroy(Renderer* renderer) {
    ...
    for (uint32_t i = 0; gl->surfaceCount > i; i++) {
        ...
        if (gl->surfaces[i] != 0 && gl->surfaces[i] != (GLuint)~0u)
            glDeleteFramebuffers(1, &gl->surfaces[i]);
    }
    free(gl->surfaces);
...
static void glBeginGUI(Renderer* renderer, int32_t guiW, int32_t guiH, int32_t portX, int32_t portY, int32_t portW, int32_t portH, int32_t targetSurfaceId) {
    ...
    if (targetSurfaceId == RENDER_TARGET_HOST_FRAMEBUFFER) {
        if (!gl->directFramebuffer) glBindFramebuffer(GL_FRAMEBUFFER, 0);
        ...
        if (!(gl->directFramebuffer && targetSurfaceId == renderer->runner->applicationSurfaceId))
...
static void glEndFrameInit(Renderer* renderer) {
    GLLegacyRenderer* gl = (GLLegacyRenderer*) renderer;
    if (gl->directFramebuffer) return;
...
static int32_t glCreateSpriteFromSurface(Renderer* renderer, int32_t surfaceID, int32_t x, int32_t y, int32_t w, int32_t h, bool removeback, bool smooth, int32_t xorig, int32_t yorig) {
    ...
    if (gl->directFramebuffer) {
        if (surfaceID != renderer->runner->applicationSurfaceId) return -1;
    } else {
...
static int32_t glLegacyCreateSurface(Renderer* renderer, int32_t width, int32_t height) {
    ...
    if (gl->directFramebuffer) {
        fprintf(stderr, "GL: surface_create(%d, %d) is unavailable without FBO support.\n", width, height);
        return -1;
    }
...
static int32_t glLegacyEnsureApplicationSurface(Renderer* renderer, int32_t width, int32_t height) {
    ...
    if (gl->directFramebuffer) {
        if (gl->surfaceCount == 0) {
            uint32_t slot = GLCommon_findOrAllocateSurfaceSlot(
                &gl->surfaces, &gl->surfaceTexture, &gl->surfaceWidth,
                &gl->surfaceHeight, &gl->surfaceCount);
            gl->surfaces[slot] = (GLuint)~0u; /* sentinel for the host framebuffer */
            gl->surfaceTexture[slot] = 0;
            id = (int32_t)slot;
            renderer->runner->applicationSurfaceId = id;
        } else if (id < 0 || (uint32_t)id >= gl->surfaceCount) {
            id = 0;
            renderer->runner->applicationSurfaceId = id;
        }
        gl->surfaceWidth[id] = width;
        gl->surfaceHeight[id] = height;
        return id;
    }
...
static void glLegacySurfaceResize(Renderer* renderer, int32_t surfaceId, int32_t width, int32_t height) {
    ...
    if (gl->directFramebuffer && surfaceId == renderer->runner->applicationSurfaceId) {
...
static void glLegacySurfaceFree(Renderer* renderer, int32_t surfaceId) {
    ...
    if (gl->surfaceTexture[surfaceId] != 0) glDeleteTextures(1, &gl->surfaceTexture[surfaceId]);
    if (gl->surfaces[surfaceId] != 0 && gl->surfaces[surfaceId] != (GLuint)~0u)
        glDeleteFramebuffers(1, &gl->surfaces[surfaceId]);
...
static bool glLegacySetRenderTarget(Renderer* renderer, int32_t surfaceId, bool implicitApplicationSurface) {
    ...
    if (gl->directFramebuffer) {
        if (surfaceId != renderer->runner->applicationSurfaceId) return false;
    } else {
        glBindFramebuffer(GL_FRAMEBUFFER, gl->surfaces[surfaceId]);
    }
...
static bool resolveSurfaceTexture(GLLegacyRenderer* gl, int32_t surfaceId, GLuint* outTexId, int32_t* outTexW, int32_t* outTexH) {
    if (0 > surfaceId || (uint32_t) surfaceId >= gl->surfaceCount) return false;
    if (gl->surfaces[surfaceId] == 0 || gl->surfaces[surfaceId] == (GLuint)~0u) return false;
...
static bool glLegacySurfaceGetPixels(Renderer* renderer, int32_t surfaceId, uint8_t* outRGBA) {
    GLLegacyRenderer* gl = (GLLegacyRenderer*) renderer;
    if (gl->directFramebuffer) {
        if (surfaceId != renderer->runner->applicationSurfaceId || !outRGBA) return false;
        glReadPixels(0, 0, gl->surfaceWidth[surfaceId], gl->surfaceHeight[surfaceId], GL_RGBA, GL_UNSIGNED_BYTE, outRGBA);
        return true;
    }
